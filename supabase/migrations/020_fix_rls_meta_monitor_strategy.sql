...
-- Also drop any policies that may have been partially created from previous attempts
DROP POLICY IF EXISTS "Anon read access on performance_rolling_metrics" ON performance_rolling_metrics;
DROP POLICY IF EXISTS "Service role write access on performance_rolling_metrics" ON performance_rolling_metrics;
DROP POLICY IF EXISTS "Anon read access on meta_interventions" ON meta_interventions;
DROP POLICY IF EXISTS "Service role write access on meta_interventions" ON meta_interventions;
DROP POLICY IF EXISTS "Anon read access on prompt_overrides" ON prompt_overrides;
DROP POLICY IF EXISTS "Service role write access on prompt_overrides" ON prompt_overrides;
DROP POLICY IF EXISTS "Anon read access on strategy_parameters" ON strategy_parameters;
DROP POLICY IF EXISTS "Service role write access on strategy_parameters" ON strategy_parameters;
DROP POLICY IF EXISTS "Anon read access on parameter_change_log" ON parameter_change_log;
DROP POLICY IF EXISTS "Service role write access on parameter_change_log" ON parameter_change_log;
